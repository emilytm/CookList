...
export function ResultsList(recipeArray){
//props needs to include the recipe array
    var resultsDisplay
    if(props.recipeArray.length >= 1){
        document.getElementById('message-display').classList.add('hidden')
        resultsDisplay = recipeArray.map(recipe => {
            return (
                <ResultItem {...recipe}/>
            )
        })
        ...
        resultsDisplay = renderNoResultState()
    }
    return (
        ...
        //  get and display one Card item per result
        //  set the innerHtml of the resultsList area to the new thing
        //if not, render the no-results state
    )
